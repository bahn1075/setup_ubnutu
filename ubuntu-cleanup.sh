...
# 컨테이너(running/stopped 모두)가 참조하는 이미지는 보존하고,
# 어떤 컨테이너에서도 참조하지 않는 이미지만 삭제
if command -v docker &> /dev/null; then
    print_info "Cleaning Docker unreferenced images..."
    USED_IMAGES=$(docker ps -a --format '{{.Image}}' | xargs -I{} docker inspect --format '{{.Id}}' {} 2>/dev/null | sort -u)
    REMOVED=0
    docker images -q | while read img; do
        FULL_ID=$(docker inspect --format '{{.Id}}' "$img" 2>/dev/null)
        if ! echo "$USED_IMAGES" | grep -q "$FULL_ID"; then
            docker rmi "$img" 2>/dev/null && REMOVED=$((REMOVED+1))
        fi
    done
    docker volume prune -f 2>&1 | tail -2 || true
    docker network prune -f 2>&1 | tail -2 || true
    docker builder prune -f 2>&1 | tail -2 || true
    print_success "Docker unreferenced images cleaned"
    echo ""
fi
